...
library(rstanarm)  # Bayesian regression: Tools for Bayesian applied regression modeling.
library(grid)
library(gt)
...
##############################################
# Prepare slope differences for plotting
##############################################

# Females vs Males without ENO
slope_females <- bsim@fixef[, "totalmates"]
slope_2males <- bsim@fixef[, "totalmates"] + bsim@fixef[, "totalmates:category2males"]
diff_f_vs_2m <- slope_females - slope_2males
ci_f_vs_2m <- round(quantile(diff_f_vs_2m, c(0.025, 0.5, 0.975)), 2)
prob_f_gt_2m <- round(mean(diff_f_vs_2m > 0), 3)

# Females vs Males with ENO
slope_3males <- bsim@fixef[, "totalmates"] + bsim@fixef[, "totalmates:category3malesENO"]
diff_f_vs_3m <- slope_females - slope_3males
ci_f_vs_3m <- round(quantile(diff_f_vs_3m, c(0.025, 0.5, 0.975)), 2)
prob_f_gt_3m <- round(mean(diff_f_vs_3m > 0), 3)

# Males without ENO vs Males with ENO
diff_2m_vs_3m <- slope_2males - slope_3males
ci_2m_vs_3m <- round(quantile(diff_2m_vs_3m, c(0.025, 0.5, 0.975)), 2)
prob_2m_gt_3m <- round(mean(diff_2m_vs_3m > 0), 3)
...
df_plot <- data.frame(
  diff = c(diff_f_vs_2m, diff_f_vs_3m, diff_2m_vs_3m),
  comparison = rep(
    c("Females vs Males without ENO",
      "Females vs Males with ENO",
      "Males without vs Males with ENO"),
    each = length(diff_f_vs_2m)
  )
)

##############################################
# Plot posterior distributions of differences
##############################################

ggplot(df_plot, aes(x = diff, fill = comparison)) +
  geom_density(alpha = 0.5) +
  geom_vline(xintercept = ci_f_vs_2m, color = "#0072B2", linetype = "dashed") +
  geom_vline(xintercept = ci_f_vs_3m, color = "#E69F00", linetype = "dashed") +
  geom_vline(xintercept = ci_2m_vs_3m, color = "#009E73", linetype = "dashed") +
  labs(
    title = "Posterior distributions of slope differences",
    x = "Slope difference",
    y = "Density",
    fill = "Comparison"
  ) +
...
year_quant <- round(quantile(as.vector(bsim@ranef$year), prob = c(0.025, 0.5, 0.975)), 2)
year_prop <- round(sum(as.vector(bsim@ranef$year) > 0) / length(as.vector(bsim@ranef$year)), 3)

year_row <- data.frame(
  Effect = "Random effect: year (15 groups",
...
summarize_diff <- function(diff_samples, label) {
  cat("Comparison:", label, "\n")
  cat("  Mean difference:", round(mean(diff_samples), 3), "\n")
  cat("  95% CI:", round(quantile(diff_samples, probs = c(0.025, 0.975)), 3), "\n")
  cat(" P(group1 > group2):", mean(diff_samples > 0), "\n\n")
}
...
plot <- ggplot(fe, aes(x = jitter(totalmates, 0.15), y = jitter(totaloff, 0.5))) +
      geom_point(shape = 17,alpha = 0.2, size=3,color = "violetred") +
...
summary_males3ENO  <- summarize_slope(slope_3males)
...
estimates_InterandSlopes <- ggplot(dens_df, aes(x = x, y = y, color = group)) +
  ...
  labs(x = "Posterior distribution of absolute slopes", y = "Density") +
...
```{r}
# Panel A
panelA <- arrangeGrob(females_plot1,nullGrob(), males_plot1, nrow = 1, widths = c(2, 0.2, 1))
panelA <- arrangeGrob(textGrob("A", x = 0, y = 1, just = c("left","top"), gp = gpar(fontsize = 20, fontface = "bold")),
                      panelA, ncol = 1, heights = c(0.1, 1))

# Panel B
panelB <- arrangeGrob(females_plot_s, nullGrob(), males_plot_s, nrow = 1, widths = c(2, 0.2, 1))
...
#Panel D
panelD<-arrangeGrob(fe_estimates_bateman_st, nullGrob(), ma_estimates_st, nrow = 1, widths = c(2, 0.2, 1))
...
ggsave("figures/black coucals/Fig1_gradients_BC.png", plot = multi_panel ,  width = 20, height = 10, dpi = 300)
...
# Calculate the proportion the for each fixed effect (column 1) that is greater than 0
round(sum(fixef(bsim)[,1]>0)/nsim,3) #0.997
round(sum(fixef(bsim)[,2]>0)/nsim,3) #0.999
round(sum(fixef(bsim)[,3]>0)/nsim,3) #0.969
round(sum(fixef(bsim)[,4]>0)/nsim,3)#0.005
...
range_totalnest_sum<- range(dataclean$totalnest_sum, na.rm = TRUE)

# Display the range of range_totalnest_sum
range_totalnest_sum

# Create a new data frame 'newdat' for plotting, with 'totalmates' ranging from 0 to 5
newdat <- expand.grid(totalnest_sum = seq(1, 5, length = 100), category = levels(dataclean$category))
...
plot <- ggplot(dataclean, aes(x = jitter(totalnest_sum, 0.15), y = jitter(totaloff, 0.5), color = category)) +
...
slope_2males <- bsim@fixef[, "totalnest_sum"] 
slope_3malesENO <- bsim@fixef[, "totalnest_sum"] + bsim@fixef[, "totalnest_sum:category3malesENO"]
...
ma_estimates_nest <- ggplot(dens_df, aes(x = x, y = y, color = group)) +
  ...
  # reference at 0
  geom_vline(xintercept = 0, linetype = "dashed") +
  labs(x = "Posterior distribution of slopes", y = "Density") +
   scale_color_manual(values = c( "orange", "blue")) +
  scale_fill_manual(values = c( "orange", "blue")) +
  scale_x_continuous(breaks = seq(floor(min(dens_df$x)),
                                  ceiling(max(dens_df$x)),
...
# 7. Calculation path coefficient (for age) females 
````{r}

#1. Path coefficients 
### 1.1 Female path coefficient (age).

# ============================
# Data Preprocessing
# ============================

#We summarise all by identity (=parent), because i have multiple data entries of some males (at least 3) across years

summary_data <- dataclean %>%
  dplyr::group_by(ring_no_lab, sex, category, age) %>%
  dplyr::summarise(
    mean_totaloff = mean(totaloff, na.rm = TRUE),
    mean_totalmates = mean(totalmates, na.rm = TRUE),
  ) %>%
  dplyr::ungroup()
...
# Separate summary data for females and males with categories 
fe_sum <- summary_data %>% filter(sex != "male") %>% droplevels()

fe_sum <- fe_sum%>%
  filter(!is.na(age)) %>%
  mutate(
    totalmates = mean_totaloff,
    totaloff = mean_totaloff,
    age = as.numeric(age),
    scaled_age = as.numeric(scale(age))
  )
...
fe_sum %>% distinct(ring_no_lab) %>% count()  # n = 51

#Prepare data ----
t.poly<- poly(fe_sum$scaled_age,2)
fe_sum$age.l<-t.poly[,1] #linear term for age
fe_sum$age.q<-t.poly[,2] #quadratic term for age

Z.l<-t.poly[,1] #linear term for age
Z.q<-t.poly[,2] #quadratic term for age
...
betaMZraw <- summary(glm(M ~ Z.l +Z.q))$coefficients["Z.l", 1]
mod       <- glm(M ~ Z.l +Z.q, data = fe_sum)
bsim      <- sim(mod, n.sims = 10000)
beta_sims <- bsim@coef[, "Z.l"]
...
betaRZraw <- summary(glm(R ~ Z.l +Z.q+ M))$coefficients["Z.l", 1]
mod       <- glm(R ~ Z.l +Z.q + M, data = fe_sum)
bsim      <- sim(mod, n.sims = 10000)
beta_sims <- bsim@coef[, "Z.l"]
...
betaRMraw <- summary(glm(R ~ Z.l +Z.q + M))$coefficients["M", 1]
mod       <- glm(R ~ Z.l +Z.q + M, data = fe_sum)
...
## 7.1 Plot Selection gradient on age controlling for M
```{r}
...
ages_repr<-ggplot()+
  geom_point(data = fedat, aes (x=age, y= totaloff, color = female_color), size=3, alpha =0.2, shape = 17, position = position_jitter(width = 0.1, height = 0.1), show.legend = FALSE ) +
  
  geom_line(data = newdat, aes(x=age,  y = fit, color = female_color, ),position = position_dodge(width = 2),   size = 1.5, show.legend = FALSE) + 
  
  geom_line(data = newdat, aes(x=age,  y = lwr, color = female_color),position = position_dodge(width = 2),   show.legend = FALSE) +
 
   geom_line(data = newdat, aes(x=age,  y = upr, color = female_color),position = position_dodge(width = 2),    show.legend = FALSE) +
    ylab("Reproductive success, mean (± 95% Crl)") +
    xlab("Age (years)") +
  labs(title = "Females (n = 51)")+
  scale_x_continuous(
  name = "Age (years)",
   limits = c(1, 3), 
  breaks = c(1, 2, 3),       # positions of ticks
  labels = c("1", "2", "3 +") # labels for each tick
)+
  custom_theme

ages_repr

ggsave("figures/black coucals/ages_fem_I.png", plot = ages_repr, 
...
age_fem <- ggplot(fe_sum, aes(x = Z, y = R)) +
  ...
  xlab("Age (variance standardized)") +
...
# Display the combined plot
print(age_fem)
age_fem<-ggarrange(age_fem, labels = c("I"))

ggsave("figures/black coucals/age_fem_I.png", plot = age_fem, 
       width = 3, height = 5, dpi = 300)

```


#8. Path coefficients Males (by category)
```{r}
# ============================
# Inspect Data
# ============================
...
ma_sum <- ma_sum %>%
  ...
  mutate(
    age = as.numeric(age),
    totaloff = as.numeric(mean_totaloff),
    totalmates = as.numeric(mean_totalmates),
...
## 8.1 Plot Selection gradient on age controlling for M
```{r}
...
ggsave("figures/black coucals/age_ma_J.png", plot = age_ma, 
       width = 3, height = 5, dpi = 300)
```


# Analysis reproductive success and age in black coucals 
## Model 
```{r}
...
```{r}
#mod <- lm(laying.rate ~ m_morph + mean_age.l + mean_age.q + year,  data = lay)
...
newdat <- expand.grid(age= seq(1,3, length=100))
newdat$age.l <- predict (t.poly, newdat$age)[,1]
newdat$age.q <- predict (t.poly, newdat$age)[,2]

head(model.matrix(mod))
Xmat <- model.matrix(~  age.l + age.q, data=newdat)
head(Xmat)
...
# Scatter plot of smoothed fitted values vs. residuals
scatter.smooth(fitted(mod), resid(mod))
abline(h=0, lty=2)
title("Tukey-Ascombe plot") # Residual vs. fitted plot indicates strong shrinkage

# QQ plot of residuals for normality assessment
qqnorm(resid(mod), main = "Normal QQ-plot of residuals")
qqline(resid(mod))

# Scatter plot of smoothed fitted values vs. square root of absolute residuals
scatter.smooth(fitted(mod), sqrt(abs(resid(mod))))
title("Residual variance vs. fitted values")

# Scatter plot of fitted values vs. observed 'number_mates' data
#plot(fitted(mod), social$number_mates, main = "Data vs. fitted values")
...
#sum(bsim@fixef[,1]<(bsim@fixef[,1]+bsim@fixef[,3]))/nsim # p(ß)=0.945
#sum(bsim@fixef[,1]<(bsim@fixef[,1]+bsim@fixef[,4]))/nsim # p(ß)=0.977
#sum(bsim@fixef[,1]+bsim@fixef[3]<(bsim@fixef[,1]+bsim@fixef[,4]))/nsim #p(ß) that 2 year-old females have less offspring than 3 year-old females, p(ß)=0.037

#sum(bsim@fixef[,1]+bsim@fixef[2]<(bsim@fixef[,1]+bsim@fixef[,2]+bsim@fixef[,5]))/nsim #p(ß) that 1 year-old males have less offspring than 2 year-old males, p(ß)=0 
#sum(bsim@fixef[,1]+bsim@fixef[2]<(bsim@fixef[,1]+bsim@fixef[,2]+bsim@fixef[,6]))/nsim #p(ß) that 1 year-old males have less offspring than 3 year-old males, p(ß)=0
#sum(bsim@fixef[,1]+bsim@fixef[2]+bsim@fixef[,5]<(bsim@fixef[,1]+bsim@fixef[,2]+bsim@fixef[,6]))/nsim #p(ß) that 2 year-old males have less offspring than 3 year-old males, p(ß)=0.032

```
##Fitting the model 
```{r}
range(fedat$age)
newdat <- data.frame(age = seq(1,3), length=100)
newdat$age.l <- predict(t.poly, newdat$age)[,1]
newdat$age.q <- predict(t.poly, newdat$age)[,2]
...
```
## Calculate counts per sex and age

```{r}
count_data <- dataclean_filtered %>%
  dplyr::group_by(sex, age) %>%
  dplyr::summarise(count = n()) %>%
  ungroup() %>%
  mutate(y_position = c(6.82, 10.37, 8.64,  6.10, 5.43, 6.23))
```

## Plotting reproductive success vs ages in black coucals
```{r}
female_color <- "violetred"   
