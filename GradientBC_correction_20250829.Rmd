...
```{r, warning=FALSE}
# Center 'totalmates' and 'totaloff' within each category
dataclean <- dataclean %>%
  group_by(category) %>%
  mutate(
    totalmates_c = totalmates - mean(totalmates, na.rm = TRUE),
    totaloff_c   = totaloff - mean(totaloff, na.rm = TRUE)
  ) %>%
  ungroup()
...
summary_pathcoeff <- data.frame(
  Component = rep(c("Linear (Z.l)", "Quadratic (Z.q)"), each = 3),
  Parameter = rep(c("Sexual Selection", "Remaining Selection", "Total Selection"), 2),
  Mean = round(c(
    mean(sexual_selection_l), mean(remaining_selection_l), mean(total_selection_l),
    mean(sexual_selection_q), mean(remaining_selection_q), mean(total_selection_q)
  ), 2),
  lowerCrl = round(c(
    round(quantile(sexual_selection_l, 0.025),2),
    quantile(remaining_selection_l, 0.025),
    quantile(total_selection_l, 0.025),
    quantile(sexual_selection_q, 0.025),
    quantile(remaining_selection_q, 0.025),
    quantile(total_selection_q, 0.025)
  ), 2),
  upperCrl = round(c(
    quantile(sexual_selection_l, 0.975),
    quantile(remaining_selection_l, 0.975),
    quantile(total_selection_l, 0.975),
    quantile(sexual_selection_q, 0.975),
    quantile(remaining_selection_q, 0.975),
    quantile(total_selection_q, 0.975)
  ), 2),
  P_prob = round(c( # Posterior probability
    mean(sexual_selection_l > 0),
    mean(remaining_selection_l> 0),
    mean(total_selection_l > 0),
    mean(sexual_selection_q > 0),
    mean(remaining_selection_q > 0),
    mean(total_selection_q > 0)
  ), 3)
)
...
(tab<-combined_summary %>%
  ...
  tab_header(
    title = paste0(
      "Table 3e. Estimates of selection on female black coucal age (Z), including sexual (xZ), remaining (rZ), and total (sZ) selection following the framework of Henshaw et al. (2018). Sample size  (n = ",  n_obs, ")."
    )
  ) %>%
  ...
  # Bold Parameter for rows from summary_pathcoeff
  tab_style(
    style = cell_text(weight = "bold"),
    locations = cells_body(
      columns = "Parameter",
      rows = source == "summary_pathcoeff"
    )
  ) %>%
  # Bold numeric columns where P_prob == 0.999
  tab_style(
    style = cell_text(weight = "bold"),
    locations = cells_body(
      columns = c("Mean", "lowerCrl", "upperCrl", "P_prob"),
      rows = P_prob == 0.999 | (P_prob >= 0.001 & P_prob <= 0.010) | (lowerCrl > 0 | upperCrl < 0)
    )
  ) %>%
...
# Convert HTML to PDF
webshot("tables/Table3e_age_BC.html", "tables/Table3e_age_BC.pdf", zoom = 2)
...
mod <- glm(totaloff ~ age.l + age.q , data = fedat)
...
ages_repr_fe<-ggplot()+
  geom_point(data = fedat, aes (x=age, y= totaloff),  color = "violetred", size=3, alpha =0.3, shape = 17, position = position_jitter(width = 0.1, height = 0.1), show.legend = FALSE ) +
...
ggsave("figures/fig3/ages_fem_I.png", plot = ages_repr_fe, 
       width = 4, height = 4, dpi = 300)

# Save the ggplot object as an RDS file
saveRDS(ages_repr_fe, file = "cache/figI.rds")
