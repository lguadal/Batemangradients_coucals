...
ages_repr<-ggplot()+
  geom_point(data = fedat, aes (x=age, y= totaloff, color = "violetred"), size=3, alpha =0.2, shape = 17, position = position_jitter(width = 0.1, height = 0.1), show.legend = FALSE ) +
  
  geom_line(data = newdat, aes(x=age,  y = fit, color = "violetred" ),position = position_dodge(width = 2),   size = 1.5, show.legend = FALSE) + 
  
  geom_line(data = newdat, aes(x=age,  y = lwr, color = "violetred"),position = position_dodge(width = 2),   show.legend = FALSE) +
 
   geom_line(data = newdat, aes(x=age,  y = upr, color = "violetred"),position = position_dodge(width = 2),    show.legend = FALSE) +
    ylab("Reproductive success, mean (± 95% Crl)") +
    xlab("Age (years)") +
  labs(title = "Females (n = 51)")+
  scale_x_continuous(
