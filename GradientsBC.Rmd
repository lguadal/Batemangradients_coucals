...
# Table 3. Path coefficients age Males (by category)
```{r}
# ============================
# Inspect Data
# ============================
...
## Fitting the model 
````{r}
# Calculate the range of values for the 'totalmates' 
...
plot <- ggplot(fe, aes(x = jitter(totalmates, 0.6), y = jitter(totaloff, 0.01))) +
      geom_point(shape = 17,alpha = 0.2, size=10,color = "violetred") +
...
```{r}
#Males
plot <- ggplot(ma, aes(x = jitter(totalmates, 0.6), y = jitter(totaloff, 0.01), color = category)) +
   geom_point(shape = 16, alpha = 0.2, size = 10)+
   labs(x = "Mating success", y = "Reproductive success") +
   ylim(0, 16)+
...
males_plot1 <- plot +
  guides(
    colour = guide_legend(
      override.aes = list(size = 6, alpha = 0.7) # smaller, transparent symbols
    )
  )
...
for (sim in 1:nsim) {
  ...
  for (cat in levels(dataclean$category)) {
    ...
    beta_interaction <- if (interaction_term %in% names(betas)) betas[interaction_term] else 0
...
ame_females   <- ame_mat[, "1females"]
ame_males2    <- ame_mat[, "2males"]
ame_males3ENO <- ame_mat[, "3malesENO"]
...
summary_females_AME   <- summarize_slope(ame_females)
summary_males2_AME    <- summarize_slope(ame_males2)
summary_males3ENO_AME <- summarize_slope(ame_males3ENO)

summaries_AME <- data.frame(
  group = c("Females", "Males without ENO", "Males with ENO"),
  rbind(summary_females_AME, summary_males2_AME, summary_males3ENO_AME),
  row.names = NULL
)
...
dens_list_AME <- list(
  Females = density(ame_females),
  `Males without ENO` = density(ame_males2),
  `Males with ENO` = density(ame_males3ENO)
)

dens_df_AME <- do.call(rbind, lapply(names(dens_list_AME), function(gr) {
  d <- dens_list_AME[[gr]]
  s <- summaries_AME[summaries_AME$group == gr, ]
  data.frame(x = d$x,
             y = d$y,
             group = gr,
             mean = s$mean,
...
modes_AME <- dens_df_AME %>%
  group_by(group) %>%
  slice_max(y, n = 1, with_ties = FALSE) %>%
  select(group, mode_x = x, mode_y = y)

estimates_InterandSlopes <- ggplot(dens_df_AME, aes(x = x, y = y, color = group)) +
  
  # ribbon between lwr and upr
  geom_ribbon(
    aes(ymin = 0,
        ymax = ifelse(x >= lwr & x <= upr, y, NA),
        fill = group),
    alpha = 0.5, color = NA
  ) +
  ...
  # short vertical line at mode (up to the peak)
  geom_segment(
    aes(x = mode_x, xend = mode_x,
        y = 0, yend = mode_y,
        color = group),
    data = modes_AME,
    linetype = "solid",
  ...
  labs( x = expression(p(beta ~ "|" ~ data)), 
       y = "Density") +
  
  scale_color_manual(values = c("violetred", "orange", "blue")) +
  scale_fill_manual(values = c("violetred", "orange", "blue")) +
  
  scale_x_continuous(breaks = seq(floor(min(dens_df_AME$x)),
                                  ceiling(max(dens_df_AME$x)),
                                  by = 2)) +
  custom_theme +
  theme(
    legend.position = "none",
...
```{r, warning=FALSE}
plotfs <- ggplot(fe, aes(x = jitter(totalmates.z, 0.6), y = jitter(mean_st_off_, 0.01))) +
   geom_point(shape = 17,alpha = 0.2, size=10,color = "violetred") +
   labs(x = expression(bar(MS)[std]), y = expression(bar(RS)[std])) +
...
fe_estimates_bateman_st <- ggplot(dens_df, aes(x = x, y = y, color = group)) +
  ...
  # reference at 0
  geom_vline(xintercept = 0, linetype = "dashed") +
  labs( x = expression(p(beta[std] ~ "|" ~ data)), y = "Density") +
...
plotmf <- ggplot(ma_om, aes(x = jitter(totalmates.z, 0.6), y = jitter(mean_st_off_, 0.01), color = category)) +
   geom_point(shape = 16, alpha = 0.2, size = 10)+
   labs(x = expression(bar(MS)[std]), y = expression(bar(RS)[std])) +
   labs(title = "Males (n = 179) ")+
  xlim(0.9, 2)+
...
## Commputing the posterior distributions
```{r}
# plot
...
# Panel A
panelA <- arrangeGrob(females_plot1,nullGrob(), males_plot1, nrow = 1, widths = c(1, 0.05, 0.5))
panelA <- arrangeGrob(textGrob("A", x = 0, y = 1, just = c("left","top"), gp = gpar(fontsize = 60, fontface = "bold")),
                      panelA, ncol = 1, heights = c(0.1, 1))

# Panel B
panelB <- arrangeGrob(females_plot_s, nullGrob(), males_plot_s, nrow = 1, widths = c(1, 0.05, 0.5))
panelB <- arrangeGrob(textGrob("B", x = 0, y = 1, just = c("left","top"), gp = gpar(fontsize = 60, fontface = "bold")),
                      panelB, ncol = 1, heights = c(0.1, 1))

#Panel C
panelC<- estimates_InterandSlopes
panelC <- arrangeGrob(textGrob("C", x = 0, y = 1, just = c("left","top"), gp = gpar(fontsize = 60, fontface = "bold")),
                      panelC, ncol = 1, heights = c(0.2, 1))

#Panel D
panelD<-arrangeGrob(fe_estimates_bateman_st, nullGrob(), ma_estimates_st, nrow = 1, widths = c(1, 0.05, 0.5))
panelD <- arrangeGrob(textGrob("D", x = 0, y = 1, just = c("left","top"), gp = gpar(fontsize = 60, fontface = "bold")),
                      panelD, ncol = 1, heights = c(0.2, 1))
...
# Stack rows
multi_panel <- grid.arrange(leftpanel, nullGrob(), rightpanel, ncol = 3, widths  = c(1, 0.18, 1))

ggsave(
  "figures/black coucals/Fig1_gradients_BC.png",
  plot = multi_panel,
  width = 48,    # ↓ narrower overall
  height = 24,   # you can adjust this too for balance
  dpi = 300,
   limitsize = FALSE,
  bg = "white"
)
...
plot <- ggplot(datclean, aes(x = jitter(totalnest_sum, 0.15), y = jitter(totaloff, 0.5), color = category)) +
   geom_point(shape = 16, alpha = 0.4, size = 10)+
...
males_nest<-plot +
  guides(
    colour = guide_legend(
      override.aes = list(size = 5, alpha = 0.7) # smaller, transparent symbols
    )
  )
...
## Computing the posterior distributions
```{r}
Xmat <- model.matrix(~ totalnest_sum * category, data = datclean)

nsim <- nrow(bsim@coef)
ncat <- length(levels(datclean$category))
ame_mat <- matrix(NA, nrow = nsim, ncol = ncat)
colnames(ame_mat) <- levels(datclean$category)

for (sim in 1:nsim) {
  
  betas <- as.numeric(bsim@coef[sim, ])
  names(betas) <- colnames(Xmat)
  
  eta <- Xmat %*% betas
  mu <- exp(eta)  # Poisson/log-link
  
  for (cat in levels(datclean$category)) {
    
    ind_cat <- datclean$category == cat
    if (sum(ind_cat) == 0) next  # skip if no rows for this category
    
    # interaction term only exists for non-reference
    interaction_term <- paste0("totalnest_sum:category", cat)
    beta_interaction <- if (interaction_term %in% names(betas)) betas[interaction_term] else 0
    
    marginal_effects <- (betas["totalnest_sum"] + beta_interaction) * mu[ind_cat]
...
summary_2males  <- summarize_slope(ame_mat[, "2males"])
summary_3malesENO  <- summarize_slope(ame_mat[, "3malesENO"])

summaries_AME <- data.frame(
  group = c("Males without ENO", "Males with ENO"),
  rbind(summary_2males, summary_3malesENO),
  row.names = NULL
)

dens_list_AME <- list(
  `Males without ENO` = density(ame_mat[, "2males"]),
  `Males with ENO` = density(ame_mat[, "3malesENO"])
)
...
# plot
ma_estimates_nest <- ggplot(dens_df_AME, aes(x = x, y = y, color = group)) +
  # ribbon between lwr and upr
  geom_ribbon(aes(ymin = 0, ymax = ifelse(x >= lwr & x <= upr, y, NA),
                  fill = group),
              alpha = 0.5, color = NA) +
  ...
  # reference at 0
  geom_vline(xintercept = 0, linetype = "dashed") +
  labs(x = expression(p(beta ~ "|" ~ data)), y = "Density") +
   scale_color_manual(values = c( "orange", "blue")) +
  scale_fill_manual(values = c( "orange", "blue")) +
  scale_x_continuous(breaks = seq(floor(min(dens_df_AME$x)),
                                  ceiling(max(dens_df_AME$x)),
...
#Panel A
panelA<- males_nest
panelA <- arrangeGrob(textGrob("A", x = 0, y = 1, just = c("left","top"), gp = gpar(fontsize = 40, fontface = "bold")),
                      panelA, ncol = 1, heights = c(0.1, 1))

#Panel B
panelB<- ma_estimates_nest
panelB <- arrangeGrob(textGrob("B", x = 0, y = 1, just = c("left","top"), gp = gpar(fontsize = 40, fontface = "bold")),
                      panelB, ncol = 1, heights = c(0.1, 1))
...
ggsave("supl/FigS1.png", plot = combinedsupl1 ,  width = 10,height = 20, dpi = 300,
         bg = "white")

```


#Figure 3 Selection gradient on age controlling for M -MALES 
```{r}
...
#Figure 3 Selection gradient on age controlling for M -FEMALES
```{r}
##### Plotting Selection on Z (controlling for M) 
