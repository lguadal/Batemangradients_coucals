...
id_sd<- apply(bsim@ranef$lab_id, c(1,3), sd)
id_quant <- round(quantile(id_sd, probs = c(0.025, 0.5, 0.975)), 2)
...
males <- ggplot(males_data, aes(x = no_mates, y = no_off)) +
  ...
  scale_x_continuous(breaks = c(1, 2), limits = c(0.90, 2)) +
custom_theme +
  ylim(0, 16) +
  guides(shape = FALSE)
...
plot <- ggplot(subset(dataclean, sex == "female"), aes(x = no_mates, y = no_off)) +
  ...
  scale_x_continuous(breaks = c(1, 2), limits = c(0.90, 2.0)) +
