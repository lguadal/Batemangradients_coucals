...
# Read the CSV file
dataclean <- read.csv("data/whitebrowedcoucal_gradients.csv")
```
...
custom_theme <- theme_classic() +
  theme(text = element_text(family = "Arial"),
        ...
        plot.title = element_text(size = 15, face = "bold"),  # bold title
...
## Table 1: Opportunity for selection (including comparisons)
````{r}
...
output <- c(
  capture.output(summarize_diff(diff_female_vs_male, "Females vs Males"))
)
...
tab<-tibble(Text = output) %>%
  gt() %>%
  tab_header(
    title = "Estimated Differences in the Opportunity for Selection of White-browed Coucals"
  )
tab

# Save as PDF
gtsave(tab, "tables/estimatediff/WBC_OppSelection_differences.pdf")
...
# Round to 3 decimals but cap extremes
prop_gt0 <- pmin(pmax(round(prop_gt0, 3), 0.001), 0.999)   # For 3 decimals
...
id_sd <- apply(bsim@ranef$lab_id, c(1,3), sd) #extract the sd among years for all the simulated values from the post distribution

id_quant <- round(quantile(id_sd, prob= c(0.025, 0.5, 0.975)),2) #among year variance, cannot be zero : standard deviation among years 
id_prop <- NA

id_row <- data.frame(
  Effect = "SD of Random effect: bird identity 83 groups",
...
id_sd <- 0.02
id_quant <- c("2.5%" = 0.01, "50%" = 0.02, "97.5%" = 0.03)  # named vector
id_prop <- NA
...
## Table 1. Bateman gradient (Poisson ditribution) - average marginal effect (AME)
````{r}
mod <- glmer(no_off ~ 
               no_mates + 
               (1|lab_id) ,  family="poisson", 
...
# Model matrix
Xmat <- model.matrix(~ no_mates, data = fe)

# Store AMEs (only one column now!)
ame_vec <- numeric(nsim)
...
for (sim in 1:nsim) {
  ...
  # AME = derivative wrt no_mates
  marginal_effects <- betas["no_mates"] * mu
  
  # Average across all observations
  ame_vec[sim] <- mean(marginal_effects)
}

# Summary
ame_summary <- data.frame(
  mean_ame = mean(ame_vec),
  lower_95 = quantile(ame_vec, 0.025),
  upper_95 = quantile(ame_vec, 0.975)
)

# Probability AME > 0 (capped)
prob_positive <- mean(ame_vec > 0)
...
tab <- ame_summary %>%
  gt() %>%
  tab_header(
    title = "Average Marginal Effect (AME) for no_mates"
  ) %>%
  fmt_number(
  ...
  cols_label(
    ...
    prob_higherzero = "P(AME > 0)"
  )

gtsave(tab, "tables/Table1_WBC_AverageMarEffects_gradients.pdf")

````


## Table 1. Bateman gradient (linear regression)
```{r}
set.seed(123)
mod <- lmer(no_off ~ 
              no_mates  +
...
##Table 1. Standardized Bateman gradient (linear regression)
```{r}
set.seed(123)
...
males <- ggplot(males_data, aes(x = no_mates, y = no_off)) +
  geom_point(alpha = 0.2, size = 10, position = position_jitter(width = 0.05, height = 0.06), color = "blue" , shape = 16) +
...
plot <- ggplot(subset(dataclean, sex == "female"), aes(x = no_mates, y = no_off)) +
  geom_point(alpha = 0.2, size = 10, shape=17, position = position_jitter(width = 0.05, height = 0.06), color = "violetred")+
   labs(x = "Mating success", y = "Reproductive success",
       color = "Sex", shape = "Sex") +
...
# --- Step 0: Model matrix ---
Xmat <- model.matrix(~ no_mates, data = fe)
nsim <- nrow(bsim@fixef)
ame_vec <- numeric(nsim)

# --- Step 1: Compute AMEs (posterior draws) ---
for (sim in 1:nsim) {
  betas <- as.numeric(bsim@fixef[sim, ])
  names(betas) <- colnames(Xmat)
  
  eta <- Xmat %*% betas        # linear predictor
  mu <- exp(eta)                # response scale
  
  marginal_effects <- betas["no_mates"] * mu
  ame_vec[sim] <- mean(marginal_effects)  # average marginal effect
}

# --- Step 2: Summarize AMEs ---
ame_summary <- data.frame(
  group = "Females",
  mean = mean(ame_vec),
  lwr = quantile(ame_vec, 0.025),
  upr = quantile(ame_vec, 0.975),
  p_gt0 = mean(ame_vec > 0)
)

# --- Step 3: Density for plotting ---
ame_density <- density(ame_vec)
dens_df_ame <- data.frame(
  x = ame_density$x,
  y = ame_density$y,
  group = "Females",
...
# --- Step 4: Plot (same style as your slope plot) ---
estimates_bateman <- ggplot(dens_df_ame, aes(x = x, y = y, color = group)) +
  ...
  geom_segment(aes(x = mode_x, xend = mode_x,
                   y = 0, yend = mode_y,
                   color = group),
               data = modes_ame, linetype = "solid", size = 1) +
  # reference at 0
...
males_st <- ggplot(subset(combined_data, sex == "male"), aes(x = no_mates.z, y = no_off_s)) +
  geom_point(alpha = 0.2, size = 10, shape=16, position = position_jitter(width = 0.05, height = 0.06), color = "blue")+
   labs(x = "Mating success", y = expression(bar(RS)[std]),
       color = "Sex", shape = "Sex") +
  labs(title = "Males (n = 79)")+
custom_theme+
  ylim(0, 4) +
...
labs(x = expression(bar(MS)[std]), y = expression(bar(RS)[std]),
 ...
 custom_theme +
  ylim(0, 4) +
 scale_x_continuous(breaks = seq(0.9, 1.9, by = 0.3)) +
  guides(shape = FALSE)
...
# Combine the plots
# Panel A
panelA <- arrangeGrob(females,nullGrob(), males, nrow = 1,  widths = c(1, 0.05, 0.5))
panelA <- arrangeGrob(textGrob("A", x = 0, y = 1, just = c("left","top"), gp = gpar(fontsize = 60, fontface = "bold")),
                      panelA, ncol = 1, heights = c(0.1, 1))

# Panel B
panelB <- arrangeGrob(females_st, nullGrob(), males_st, nrow = 1, widths = c(1, 0.05, 0.5))
...
ggsave("figures/whitebrowed coucals/Fig2_gradients_WC.png", plot = multi_panel ,
  width = 44,    # ↓ narrower overall
  height = 23,   # you can adjust this too for balance
  dpi = 300,
   limitsize = FALSE,
  bg = "white")
