...
# Read the CSV file
morpho <- read.csv("data/morpho_BC.csv")
...
t.poly<- poly(fe_sum$Z,2)
fe_sum$Z.l<-t.poly[,1] #linear term for age
fe_sum$Z.q<-t.poly[,2] #quadratic term for age


##### Plotting Selection on Z (controlling for M) 

mod <- glm(R ~ Z.l + Z.q , data = fe_sum)
...
smi_fem<-ggplot()+
  geom_point(data = fe_sum, aes (x=Z, y= R),  color = "violetred", size=3, alpha =0.3, shape = 17, position = position_jitter(width = 0.1, height = 0.1), show.legend = FALSE ) +
...
ggsave("figures/fig3/alternative_smi_fem_A.png", plot = smi_fem, 
       width = 3, height = 4, dpi = 300,
       bg="transparent")

# Save the ggplot object as an RDS file
saveRDS(smi_fem, file = "cache/alternativefigA.rds")
...
ggsave("figures/fig3/alternative_tarsus_fem_C.png", plot = tarsus_fem, 
       width = 3, height = 4, dpi = 300,
       bg="transparent")

# Save the ggplot object as an RDS file
saveRDS(tarsus_fem, file = "cache/alternativefigC.rds")
...
ggsave("figures/fig3/alternative_tail_fem_E.png", plot = tail_fem, 
       width = 3, height = 4, dpi = 300,
       bg="transparent")

# Save the ggplot object as an RDS file
saveRDS(tail_fem, file = "cache/alternative_figE.rds")
...
ggsave("figures/fig3/alternative_smb_fem_G.png", plot = smb_fem, 
       width = 3, height = 4, dpi = 300,
       bg="transparent")

# Save the ggplot object as an RDS file
saveRDS(smb_fem, file = "cache/alternative_figG.rds")
...
### 2.3 Male path coefficient (scaled body mass).
```{r}
# ============================
# Inspect Data
# ============================
...
#### 2.3.1 Plot Selection gradient on scaled body mass controlling for M
```{r}
